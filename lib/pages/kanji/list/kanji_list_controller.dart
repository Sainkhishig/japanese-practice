...
class KanjiListController extends StateNotifier<KanjiState> {
  ...
  //#region ==================== accessor ====================
  // List<KanjiState>? get lstPlan => state.resultList;
  ...
  Future<void> setMasterData() async {
    ...
    // await api.getKanji("", KanjiType.room.id);
    // masterRoomType = await api.gqlSearchRoomTypeMaster();
    // await api.gqlSearchCancellationPolicyMaster("");

    // await api.getqu("", KanjiType.room.id);
  }

  // List<KanjiState>? get filteredCancellationPolicy {
  ...
  // Iterable<KanjiState> list = [...lstPlan!];
  ...
  void update(String uKey) {
    var _todoQuery = _database.child("/kanji");
